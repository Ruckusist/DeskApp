...
class Session:
    def __init__(self, server, stream, addr, verbose):
        ...
        # use the same sink/quiet settings as server.print
    ...
    def main_loop(self):
        while True:
            ...
            try:
                ...
                # these kinds of things happen; route quietly to logger
                try:
            ...
            if self.server is not self:
                if getattr(self.server, 'should_shutdown', False):
                    # Avoid calling back into server while shutting down
                    pass
                else:
