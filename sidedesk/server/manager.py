...
class _ServerManager:
    """Lightweight wrapper to run deskapp.server.Server in a background thread.

    Contract:
    - start(host:str='localhost', port:int=28080, verbose:bool=True) -> bool
    - stop() -> bool
    - restart() -> bool
    - is_running() -> bool
    ...
    def start(self, host: str = 'localhost', port: int = 28080, verbose: bool = True, sink=None, quiet: bool = True):
        ...
        with self._lock:
            if self.is_running():
                ...
                return True, None
            # Preflight bind check to give a clearer error
    ...
    def restart(self) -> bool:
        ...
        if not self.stop():
            return False
        # small delay to ensure socket release on some OSes
        time.sleep(0.2)
