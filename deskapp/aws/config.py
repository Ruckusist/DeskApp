...
def find_config_path() -> Optional[Path]:
    ...
    if env_path:
        ...
        if p.exists():
            return p
    # 2) Local project .config path (repo)
    # Try current working directory (useful when running from project root)
    cwd_local = Path.cwd() / ".config/deskapp/aws/config.toml"
    if cwd_local.exists():
        return cwd_local
    # Try walking up from this file to find a repo-local .config
    here = Path(__file__).resolve()
    for parent in [here.parent, *here.parents]:
        candidate = parent / ".config/deskapp/aws/config.toml"
        if candidate.exists():
            return candidate
    ...
    if xdg_path.exists():
        return xdg_path
    # 4) Legacy project-local fallback (within package dir)
